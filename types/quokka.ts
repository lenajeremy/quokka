...
type HookSuffix<T> =
    T extends QuokkaApiQuery<any, any> ? "Query" :
    T extends QuokkaApiMutation<any, any> ? "Mutation" :
    never;

type HookFunction<T> =
    T extends QuokkaApiQuery<infer Takes, any> ? (val: Takes) => QuokkaApiQueryParams :
    T extends QuokkaApiMutation<infer Takes, any> ? (val: Takes) => QuokkaApiMutationParams :
    never;
