...
function resolveRequestParameters(apiInit, endpointParams, getState) {
    ...
    if (apiInit.prepareHeaders) {
        mergeHeaders([apiInit.prepareHeaders(getState, new Headers()), endpointParams.headers], headers);
    }
    else {
 ...
 * parameters would always have the same form so they are deduped correctly.
 *
 * Examples:
 * ```javascript
...
function hasMatchingTags(mTags, qTags, res) {
    if (!mTags || !qTags)
        return false;
    var mutationTagsValue = (0, types_1.isArrayTag)(mTags) ? mTags : mTags(res);
    var queryTagsValue = (0, types_1.isArrayTag)(qTags) ? qTags : qTags(res);
    if (mutationTagsValue.length == 0 && queryTagsValue.length == 0)
        return true;
    for (var _i = 0, mutationTagsValue_1 = mutationTagsValue; _i < mutationTagsValue_1.length; _i++) {
        var mTag = mutationTagsValue_1[_i];
        for (var _a = 0, queryTagsValue_1 = queryTagsValue; _a < queryTagsValue_1.length; _a++) {
            var qTag = queryTagsValue_1[_a];
            if (deepCompare(mTag, qTag)) {
                return true;
            }
        }
    }
