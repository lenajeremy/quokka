import { isArrayTag, type AnyFunction, type TagType } from "./types";
import type {
  CreateApiOptions,
...
function isQueryParams<T, R>(
  p: QuokkaApiMutationParams<T, R> | QuokkaApiQueryParams<T, R>,
): p is QuokkaApiQueryParams<T, R> {
...
export function resolveRequestParameters<
  TagString,
  T extends
    | QuokkaApiMutationParams<TagString, any>
    | QuokkaApiQueryParams<TagString, any>,
>(
  ...
  if (apiInit.prepareHeaders) {
    mergeHeaders(
      [apiInit.prepareHeaders(getState, new Headers()), endpointParams.headers],
      headers,
    );
  } else {
 ...
 * */
export async function generateRequestKey<T, R>(
  requestParams: QuokkaApiQueryParams<T, R> | QuokkaApiMutationParams<T, R>,
): Promise<string> {
