...
export function resolveRequestParameters<
  TagString,
  T extends
    | QuokkaApiMutationParams<TagString>
    | QuokkaApiQueryParams<TagString>,
>(
 ...
 * */
export async function generateRequestKey<T>(
  requestParams: QuokkaApiQueryParams<T> | QuokkaApiMutationParams<T>,
): Promise<string> {
...
export function hasMatchingTags<T>(
  mTags?: TagType<T>,
  qTags?: TagType<T>,
): boolean {
  ...
  for (const mTag of mTags) {
    for (const qTag of qTags) {
      if (
        deepCompare(
          mTag instanceof Function ? mTag() : (mTag as any),
          qTag instanceof Function ? qTag() : (qTag as any),
        )
      ) {
        return true;
      }
